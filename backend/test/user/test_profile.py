...
def test_user_info_patch(client, test_country_native, test_region, test_city):
    test_user_info['document']['insurance_policy'] = '123-456-789 01'
    resp = client.patch('/personal', json=test_user_info)
    assert resp.status_code == 200

    resp = client.get('/personal')
    assert resp.status_code == 200
    assert resp.json['document']['insurance_policy'] == '123-456-789 01'

    import copy
    test_info = copy.deepcopy(test_user_info)
    test_info['document']['insurance_policy'] = '987-654-321 10'
    ...
    resp = client.get('/personal')
    assert resp.status_code == 200
    assert resp.json['document']['insurance_policy'] == '987-654-321 10'
