...
# noinspection DuplicatedCode
def test_results_with_variant(client, create_user_with_answers_without_plain):
    ...
    resp = client.get(f'/contest/{contest_id}/user/{user_id}/results')
    assert resp.status_code == 409
    ...
    assert resp.json['contest_id'] == contest_id
    assert resp.json['user_id'] == user_id
    assert resp.json['total_points'] == 14 * 3
